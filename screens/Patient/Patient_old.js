...
const Patient = ({navigation,route }) => {
  ...
  useEffect(() => { 
   ...
   //const intervalId = setInterval(() => {
    ...
    fetch('http://'+host+'/dashboard/CheckticketPatient.php',{method: 'POST',
    headers: {
    ...
    .then(data => {
      for (let i = 0; i < data['data'].length; i++) {
        ...
        Services[i]=data['data'][i]['tckt_service'];
        Status[i]=data['data'][i]['tckt_status'];
       Dates[i]=data['data'][i]['tckt_date'];
        TicketId[i]=data['data'][i]['tckt_id'];
          //    console.log(' ticekt  Number [' ,i ,']: ', Services[i] ,' | ' , Status[i] , '|' , Dates[i] );
                  }
